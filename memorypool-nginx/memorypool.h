...
class MemoryPool {
 public:
  ...
  void* allocWithAligned(size_t);
  void* allocWithoutAlign(size_t);

  void* allocLargeWithoutSearch(size_t, size_t);
  long largeFree(void*);

  //   pool_t* Get() const { return pool_.get(); }
  pool_t* get() const { return pool; }

 private:
  void* allocSmall(size_t, unsigned long);
  void* allocLarge(size_t);
  void* allocBlock(size_t);

  // std::unique_ptr<pool_t> pool_;
  pool_t* pool;
};
